import entities.Gringotts.Wizzard;
import entities.HospitalDB.*;
...
//        runHospitalDB();
    ...
    private  static void   runHospitalDB(){
    ...
    private static void runUniversitySystem(){
        //TODO change in pom.xml persistenceUnitName and jdbc property
        EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("xxxxxx");
        EntityManager entityManager = entityManagerFactory.createEntityManager();

        entityManager.getTransaction().begin();

        //TODO add some logic for testing
    ...
    private static void runGringotts() {
        EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("h_code_first");
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        Wizzard wizzard = new Wizzard();
        wizzard.setFirstName("asdsss");
        wizzard.setLastName("asddddd");
        wizzard.setAge(45);
        wizzard.setDepositAmount(new BigDecimal(4321));

        entityManager.getTransaction().begin();
        entityManager.merge(wizzard);
        entityManager.flush();
        entityManager.getTransaction().commit();
    }
